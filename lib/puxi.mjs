...
class ScrollManager extends EventManager {
    constructor(target, preventDefault = true) {
        ...
        this.onMouseScrollImpl = (e) => {
            ...
            this.scale = Math.min(Math.max(0, this.scale), 100);
            //console.log(this.scale)
            
            //target.scrollBars[0].handle.contentContainer.y = this.scale
            //console.log(this.scale/100)
           //console.log()
            let scrollHeight = target.scrollBars[0].contentContainer.children[0].height
            scrollHeight = (scrollHeight/100)*(this.scale)
            //console.log(scrollHeight)
            target.scrollBars[0].handle.contentContainer.y = scrollHeight

            this.scale += e.deltaY * 0.01;
            target.forcePctPosition('y', this.scale/100)
            target.scrollBars[0].percentValue = this.scale/100
            //console.log(target.scrollBars[0].percentValue)
...
class ScrollBar extends Slider {
    ...
    initialize() {
        super.initialize();
        this.decimals = 20; // up decimals to trigger ValueChanging more often
        // eslint-disable-next-line @typescript-eslint/no-unused-vars

        this.maxHeight = this.scrollingContainer.contentContainer.height
        this.maskedHeight = this.scrollingContainer.height
        ...
        this.on('changing', () => {
            ...
            var fullHeight = this.scrollingContainer.contentContainer.height
            let multiplier = ((fullHeight/maskedHeight) - 1)
            ...
            if(multiplier >= 1) {
                var scrollingOffset = (this.maxHeight/this.maskedHeight)*multiplier;
            } else {
                var scrollingOffset = (this.maxHeight/this.maskedHeight)
            }
            
            let percent = this.percentValue*100
            let offsetVertical = (this.maxHeight/100) * percent

            /*if(this.percentValue > 0.85) {
                return
            }
            
            result = (this.percentValue * -scrollingOffset)
            console.log(result)*/
...
class ScrollWidget extends InteractiveGroup {
    ...
    constructor(options = {}) {
        ...
        this.forcePctPosition = (direction, pct) => {
            ...
            const container = this.innerContainer.contentContainer
    
            let offset = ((this.innerContainer.contentContainer.height - bounds.height + 20) * pct) *-1

            if (this.scrollY && direction === 'y') {
                container.position[direction] = offset
            } 
        ...
        if (options.scrollBars && this.scrollY) {
            ...
            verticalScrollBar.paddingTop = 70
            verticalScrollBar.paddingBottom = 80
    ...
    getInnerBounds(force) {
        ...
        if (force || performance.now() - this.boundCached > 1000) {
            this.innerContainer.contentContainer.getLocalBounds(this.innerBounds);
...
class TextInput extends FocusableWidget {
    ...
    constructor(options) {
        ...
        this.drawSelectionRect = (x, y, w, h) => {
            this.selection.beginFill(`0xFFFF00`, 1);
