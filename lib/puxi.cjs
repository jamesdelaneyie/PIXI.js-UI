...
function initMockDOMInput() {
    ...
    if (typeof mockDOMInput === 'undefined') {
        ...
        mockDOMForm = document.createElement('FORM');

        mockDOMSubmit = document.createElement('INPUT');
        mockDOMInput = document.createElement('INPUT');

        mockDOMSubmit.setAttribute('type', 'submit')
        mockDOMSubmit.setAttribute('onclick', 'sendVal(this.form)')
        mockDOMSubmit.setAttribute('value', 'tester')
        mockDOMSubmit.setAttribute('style', 'position:fixed; left:-10px; top:-10px; width:0px; height: 0px;');

        mockDOMInput.setAttribute('type', 'text');
        mockDOMInput.setAttribute('id', '_pui_tempInput__');
        mockDOMInput.setAttribute('style', 'position:fixed; left:-10px; top:-10px; width:0px; height: 0px;');

        mockDOMForm.appendChild(mockDOMInput)
        mockDOMForm.appendChild(mockDOMSubmit)

        //document.body.appendChild(mockDOMForm);
        document.body.appendChild(mockDOMInput);
        document.body.appendChild(mockDOMSubmit);
    }
}
/**
 * An UI text object
